...
export default function FerrySection() {
  ...
  return (
    <section id="ferry" ref={sectionRef} className="py-24 px-6 bg-[#FBFBFD]">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-[#515154] mt-3 text-[15px] leading-relaxed">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-8">
          {routes.map((r, i) => (
            <motion.article
              ...
              className="bg-white border border-[#E8E8ED] rounded-3xl p-5"
              style={{ boxShadow: "0 4px 24px rgba(0,0,0,0.05)" }}
            >
              <div className="flex items-start gap-3 mb-3">
                <div className="w-10 h-10 rounded-xl bg-[#0071E3]/10 flex items-center justify-center shrink-0">
                  <Ship className="w-5 h-5 text-[#0071E3]" aria-hidden="true" />
                </div>
                <div className="min-w-0">
                  <p className="text-[13px] text-[#515154] flex items-center gap-1.5">
              ...
              <p className="text-[12px] text-[#515154] mb-3">{r.operators}</p>

              <div className="flex items-center gap-4 text-[12px] text-[#515154] mb-3">
                <span className="flex items-center gap-1.5">
                  <Clock className="w-3.5 h-3.5" aria-hidden="true" /> {r.crossing}
                </span>
                <span className="font-semibold text-[#1D1D1F]">{r.price}</span>
              </div>

              <p className="text-[12px] text-[#0071E3]">{r.note}</p>
            </motion.article>
          ))}
        </div>
        ...
        <motion.div
          ...
          className="bg-[#F5F5F7] rounded-3xl p-6"
        >
          ...
          <ul className="grid sm:grid-cols-2 gap-2.5 mb-4">
            {tips.map((t) => (
              <li key={t} className="flex items-start gap-2 text-[12px] text-[#515154] leading-relaxed">
                <ArrowRight className="w-3.5 h-3.5 text-[#0071E3] shrink-0 mt-0.5" aria-hidden="true" />
                {t}
              </li>
            ))}
          </ul>
          <p className="flex items-start gap-2 text-[11px] text-[#515154] border-t border-[#E5E5EA] pt-3">
